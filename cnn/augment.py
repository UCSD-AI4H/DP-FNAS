...
def worker(gpu, ngpus_per_node, config_in):
    ...
    if config.gpu is not None:
        ...
        logger_name = "{}.{}-{:d}-{:d}.aug.log".format(config.name, jobid, procid, gpu)
        ploter_name = "{}-{:d}-{:d}".format(jobid, procid, gpu)
        ck_name = "{}-{:d}-{:d}".format(jobid, procid, gpu)
    else:
        writer_name = "tb.{}-{:d}-all".format(jobid, procid)
        logger_name = "{}.{}-{:d}-all.aug.log".format(config.name, jobid, procid)
    ...
    for epoch in range(config.epochs):
        ...
        train(logger, writer, device, config,
              train_loader, model, optimizer, criterion, epoch)
        lr_scheduler.step()
...
def train(logger, writer, device, config, train_loader, model, optimizer, criterion, epoch):
...
def validate(logger, writer, device, config, valid_loader, model, criterion, epoch, cur_step):
    top1 = utils.AverageMeter()
    top5 = utils.AverageMeter()
    losses = utils.AverageMeter()
