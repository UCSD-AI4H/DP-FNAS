...
class Architect():
    ...
    def __init__(self, net, w_momentum, w_weight_decay):
        ...
        if type(net) == nn.parallel.DistributedDataParallel:
            ...
            self.v_net_in = copy.deepcopy(net)
            self.v_net = self.v_net_in.module
        else:
            self.net = net
    ...
    def unrolled_backward(self, config_in, trn_X, trn_y, val_X, val_y, xi, w_optim):
        ...
        with torch.no_grad():
            for alpha, da, h in zip(self.net.alphas(), dalpha, hessian):
                if not config.dist_privacy:
                    alpha.grad = da - xi*h
                else: # add noise on hessian
                    # g_cuda = torch.Generator(device=torch.device('cuda', config.gpu))
                    # g_cuda.manual_seed(config.seed)
                    gaussian_noise = torch.normal(0,
                                                config.var_sigma * max_norm,
                                                h.shape,
                                                device=torch.device('cuda', config.gpu),
                                                # generator=g_cuda,
                                            )
                    # print("before: ", alpha.grad)
                    alpha.grad = da - xi* (h + config.max_hessian_grad_norm * gaussian_noise)
                    # print("after: ", alpha.grad)
