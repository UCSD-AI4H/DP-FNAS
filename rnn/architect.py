...
class Architect(object):
    ...
    def _compute_unrolled_model(self, hidden, input, target, eta):
        ...
        # for param_name, param_iter in self.model.named_parameters():
        #     print(param_name, ':', param_iter.shape)
        grads = torch.autograd.grad(loss, self.model.parameters())
        clip_coef = _clip(grads, self.network_clip)
        dtheta = _concat(grads).data + self.network_weight_decay * theta
        unrolled_model = self._construct_model_from_theta(
            theta.sub(eta, dtheta))
        return unrolled_model, clip_coef

    def step(self, hidden_train, input_train, target_train, hidden_valid,
             input_valid, target_valid, network_optimizer, unrolled):
        eta = network_optimizer.param_groups[0]['lr']
        self.optimizer.zero_grad()
        if unrolled:
            hidden = self._backward_step_unrolled(hidden_train, input_train,
                                                  target_train, hidden_valid,
                                                  input_valid, target_valid,
                                                  eta)
        else:
            hidden = self._backward_step(hidden_valid, input_valid,
                                         target_valid)
        self.optimizer.step()
        return hidden, None
    ...
    def _backward_step_unrolled(self, hidden_train, input_train, target_train,
        ...
        implicit_grads = self._hessian_vector_product(vector,
                                                      ...
                                                      target_train,
                                                      r=1e-2)
        if self.config.dist_privacy:
            max_norm = float(self.config.max_hessian_grad_norm)
            gaussian_noise = torch.normal(0,
                                          self.config.var_sigma * max_norm,
                                          implicit_grads.shape,
                                          device=torch.device('cuda', self.config.gpu),
                                          )
            implicit_grads = implicit_grads + gaussian_noise
    ...
    def _hessian_vector_product(self, vector, hidden, input, target, r=1e-2):
        ...
        # debug
        # for param_item in self.model.arch_parameters():
        #     if not param_item.requires_grad:
        #         print(param_item)
