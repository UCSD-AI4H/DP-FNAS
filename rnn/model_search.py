...
class DARTSCellSearch(DARTSCell):
    def __init__(self, ninp, nhid, dropouth, dropoutx):
        super(DARTSCellSearch, self).__init__(ninp,
                                              nhid,
                                              dropouth,
                                              dropoutx,
                                              genotype=None)
    ...
    def cell(self, x, h_prev, x_mask, h_mask, weights_in):
        s0 = self._compute_init_state(x, h_prev, x_mask, h_mask)
        s0 = self.bn(s0)
        probs = F.softmax(weights_in, dim=-1)
        ...
        for i in range(STEPS):
            ...
            ch = masked_states.view(-1, self.nhid).mm(self._Ws[i]).view(
                i + 1, -1, 2 * self.nhid)
            ...
            for k, name in enumerate(PRIMITIVES):
                ...
                s += torch.sum(
                    probs[offset:offset + i + 1, k].unsqueeze(-1).unsqueeze(-1)
                    * unweighted,
                    dim=0)
            s = self.bn(s)
...
class RNNModelSearch(RNNModel):
    def __init__(self, *args):
        super(RNNModelSearch, self).__init__(*args,
                                             ...
                                             genotype=None)
        self._args = args
        # self._arch_parameters_list = nn.ParameterList()
    ...
    #     # weights_data = nn.Parameter(torch.randn(k, len(PRIMITIVES), requires_grad=True).mul_(1e-3))
    #     # weights_data = torch.randn(k, len(PRIMITIVES)).mul_(1e-3)
    #     weights_data = nn.Parameter(1e-3 * torch.randn(k, len(PRIMITIVES)))
    #     self.weights = weights_data.cuda()
    #     print("weights shape:", self.weights.shape)
    #     self._arch_parameters = [self.weights]
    #     for rnn in self.rnns:
    #         print(rnn)
    #         # print("rnn.weights shape:", rnn.weights.shape)
    #         rnn.weights = self.weights
    #         print(self)
    # def _initialize_arch_parameters(self):
    #     k = sum(i for i in range(1, STEPS + 1))
    #     self.weights = nn.Parameter(1e-3 * torch.randn(k, len(PRIMITIVES)))
    #     for rnn in self.rnns:
    #         print(rnn)
    #         self._arch_parameters_list.append(rnn.weights)
    def _initialize_arch_parameters(self):
        k = sum(i for i in range(1, STEPS + 1))
        self.weights = nn.Parameter(1e-3 * torch.randn(k, len(PRIMITIVES)))
        self._arch_parameters = [self.weights]
    ...
    # def arch_parameters(self):
    #     for p in self._arch_parameters_list:
    #         yield p
    ...
    def genotype(self):
        def _parse(probs):
            ...
            for i in range(STEPS):
                ...
                j = sorted(range(i + 1),
                           key=lambda x: -max(W[x][k] for k in range(len(W[
                               x])) if k != PRIMITIVES.index('none')))[0]
                k_best = None
